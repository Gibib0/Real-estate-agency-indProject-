...
namespace Persistence.JSONImplementations
{
    public class JSONPropertyService : IPropertyService
    {
        ...
        public bool DeleteProperty(Guid id)
        {
            ...
            if (propertyToRemove == null) return false;
        ...
        public IEnumerable<Property> GetPropertiesByFilters(PropertyFilter filter)
        {
            ...
            if (!string.IsNullOrWhiteSpace(filter.Landmark))
                query = query.Where(p => p.Landmarks != null && p.Landmarks.Any(lm => lm.Name.Equals(filter.Landmark, StringComparison.OrdinalIgnoreCase) && (!filter.MaxTravelTime.HasValue || lm.TravelTimeMinutes <= filter.MaxTravelTime.Value)));
